...
// Script designed to recognize foci from EARLY ZYOGTENE cells from testis spreads, from composite 16-bit 3-Channel images. 
...
for (i=0; i<NumberOfFiles; i++) {
	run("Bio-Formats Importer", "open=["+MyDirectory+ListOfFiles[i]+"] color_mode = Composite view=Hyperstack stack_order=XYCZT stitch_tiles");
		...
		run("Duplicate...", " ");
		selectWindow("Sycp3");
		run("Auto Threshold", "method=Mean white"); // Note this method of SYCP3 detection differs per stage. 
		run("Convert to Mask");
